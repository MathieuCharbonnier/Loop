...
def validate_parameters(neurons_population, connections, spindle_model, biophysical_params, muscles_names, number_muscles, ees_recruitment_profile):
        """
        Validates the configuration parameters for the neural model.
        
        Checks for consistency between neurons, connections, spindle model,
        ...
        defined_neurons = set(neurons_population.keys())
        ...
        if "inh" in neuron_types:
            if "E_inh" not in biophysical_params or "tau_i" not in biophysical_params:
        ...
        for connection_pair in connections:
            ...
            if number_muscles == 2:
                if not any(muscle in pre_neuron for muscle in muscles_names) and '_' not in pre_neuron:
                ...
                if not any(muscle in post_neuron for muscle in muscles_names) and '_' not in post_neuron:
            ...
            if pre_neuron not in neurons_population and pre_type not in neurons_population:
                issues["errors"].append(f"Neuron '{pre_neuron}' used in connection {connection_pair} but not defined in the neurons population")
            
            if post_neuron not in neurons_population and post_type not in neurons_population:
        ...
        if ees_recruitment_profile:
            # Check if all required neuron types have recruitment parameters
            for neuron_type in neuron_types:
                if neuron_type in ["Ia", "II", "MN"] and neuron_type not in ees_recruitment_profile:
        ...
        if 'tau_i' in biophysical_params:
            value = biophysical_params['tau_i']
        ...
        if 'E_inh' in biophysical_params:
            value = biophysical_params['E_inh']
