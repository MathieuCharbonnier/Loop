...
class AnalyzerEES:
    ...
    def get_results_summary(self, results=None):
        ...
        -----------
        results : dict, optional
            Results dictionary (uses self.results if None)
        ...
        if results is None:
            if self.results is None:
                raise ValueError("No results available! Run an analysis method first.")
            results = self.results
        
        summary = {
            'parameter_name': results['param_name'],
            'parameter_label': results['param_label'],
            'parameter_values': results['param_values'],
            'num_muscles': results['num_muscles'],
            'muscle_names': results['muscle_names'],
            'associated_joint': results['associated_joint'],
            'num_simulations': len(results['simulation_data'])
        }
        
        return summary
    
    def export_results(self, results=None, filename="ees_analysis_results.pkl"):
        """
        ...
        if results is None:
            if self.results is None:
                raise ValueError("No results to export! Run an analysis method first.")
            results = self.results
